...
- Buyer-side: apply for an account → admin approves with a pricing tier, an advance %, and a credit limit → place tiered-price orders against a small wholesale catalog (paying the advance % via real Razorpay at order time) → track status, including real Shadowfax AWB tracking once dispatched → download invoices → view a running ledger/statement.
- Admin-side: review/approve/reject/suspend accounts, manage price tiers and the wholesale catalog, manage orders through a status workflow (including booking a real Shadowfax shipment), issue invoices and credit notes, record the remainder payment/adjustments, and watch FY turnover against the GST registration threshold.
...
| `B2BOrder.js` | Deliberately separate from retail `Order` — different fields (cases, dispatch/LR, credit hold, invoice link) and a different status workflow. Items/billing/shipping are snapshots taken at placement time, never re-read live later. `advancePercent`/`advanceAmount`/`remainingAmount`/`remainingDueDate`/`razorpayOrderId`/`razorpayPaymentId` snapshot the advance payment (see Advance/remainder payment below). `courier` (via the shared `models/schemas/courierSchema.js`, also used by retail `Order`) holds real Shadowfax shipment state once an admin books one (see Shadowfax shipment below) — separate from the older free-text `dispatch` sub-object, which stays as manual transporter/LR/vehicle notes regardless of whether a real AWB exists. |
...
| `b2bCredit.js` | `getOutstanding` (derived from `LedgerEntry`), `getOpenOrderValue`, `getOverdueSummary` (FIFO credit allocation across unpaid invoices), `getCreditSummary`, `shouldHold(account, amountAtRisk)` — no more special-casing by payment type; callers pass whatever amount is genuinely still at risk (the full payable before an order exists, or `payable - advanceAmount` once one does), and a 100%-advance order naturally never holds since its remaining amount is 0. |
...
| `b2bInvoicing.js` | `issueInvoiceForOrder` — the transactional invoice-issuance core (see below). Also exports `REMAINDER_DUE_DAYS` (14) and `advanceNoteFor(order)` (the "Advance: X% paid (₹Y)" / "Full amount on credit" line both invoice-PDF controllers show). |
| `b2bOrderCreation.js` | `createB2BOrderForUser` — the transactional order-placement core (see Advance/remainder payment below), mirroring retail's `utils/orderCreation.js` shape. |
...
### Advance/remainder payment (replaces the old fixed payment-terms model)

Every `BusinessAccount` has a single `advancePercent` (0-100, admin-set on approval) instead of the earlier fixed `prepaid`/`net7`/`net15`/`net30` choice. At order placement, `advancePercent`% of the priced payable is collected via a **real Razorpay payment**, and the rest (`remainingAmount = payable - advanceAmount`, never independently rounded) is tracked as due a fixed **14 days after placement** (`remainingDueDate`, `REMAINDER_DUE_DAYS` in `utils/b2bInvoicing.js`) — the same window for every business, only the percentage varies. The remainder is still collected **manually**, recorded through the existing Ledger "Record Payment" flow when it actually arrives — there is no auto-charge later (that would need stored card tokens/subscriptions, out of scope).

- **Flow mirrors retail checkout's real-money invariants exactly**: `POST /orders/payment/create-order` (`b2bPaymentController.createAdvancePaymentOrder`) re-prices via `priceB2BOrder`, computes `advanceAmount`, creates a Razorpay order for that amount, and writes a `VerifiedPayment` row — the **same model retail uses**, reused completely unmodified. The client then opens Razorpay Checkout for `advanceAmount` only and calls the **existing, unmodified** `POST /api/payment/verify` (HMAC-checked, ownership-checked) — no B2B-specific verify endpoint exists or is needed.
- **`utils/b2bOrderCreation.js`'s `createB2BOrderForUser`** is the only place a verified B2B payment gets consumed into an order (mirrors `utils/orderCreation.js`'s `createOrderForUser`): re-prices fresh, recomputes `advanceAmount` from the account's *current* `advancePercent` and cross-checks it against `verifiedPayment.amount` (rejects "pricing changed" on a mismatch — e.g. a catalog price changed between quote and payment), atomically claims the payment (`findOneAndUpdate({_id, consumedAt: null}, {consumedAt: new Date()})` — the same double-spend guard retail uses), then creates the `B2BOrder` + writes one advance `LedgerEntry` (`type: 'payment', credit: advanceAmount, method: 'razorpay', refModel: 'B2BOrder'`) **inside one transaction** (same "order + its money-entry must commit together" rule `issueInvoiceForOrder` already follows).
- **A 0%-advance account skips payment entirely** — `createB2BOrderForUser` places the order directly with no `VerifiedPayment` involved at all, the same shape as retail's COD path. `B2BQuickOrderPage`'s confirm step calls `b2bAPI.createAdvancePaymentOrder`+Razorpay only when the quote's `advanceAmount > 0`; otherwise it calls `b2bAPI.placeOrder` directly. The confirm modal always shows the pay-now/due-later breakdown before anything is charged.
- **Credit-hold** (`shouldHold`, see `b2bCredit.js` above) is checked against `remainingAmount`, not the full payable — the advance is already guaranteed paid before the order exists, so it's not really "at risk." `b2bAdminOrderController.updateOrderItems` (admin editing a still-`placed` order's quantities) recomputes `remainingAmount = max(0, newPayable - advanceAmount)` the same way — the already-collected advance never changes, only what's still owed does.
- **`server/scripts/migrateB2BPaymentTerms.js`** (`npm run b2b:migrate-payment-terms`, dry-run by default) backfills any account created before this change: old `paymentTerms` `prepaid → advancePercent: 100`, `net7`/`net15`/`net30 → advancePercent: 0` (closest honest equivalent — no advance was ever collected under credit terms).
- **Known limitation, by design**: self-service cancellation of a `placed` order (`POST /orders/:id/cancel`) does not auto-refund an already-collected advance. The ledger credit stays fully visible (admin reconciles manually, same "manual" principle as the remainder itself) — automatic Razorpay refunds were out of scope for this change.
...
### Shadowfax shipment (real AWB, admin-triggered — Phase C)

Mirrors the retail Shadowfax integration (§28) exactly, reusing `services/shadowfaxService.js` completely unmodified except one additive option (`createWarehouseOrder(order, { locationType })`, defaulting to `'residential'`; B2B always passes `'commercial'`).

- **`server/controllers/b2bShippingController.js`** (`createB2BShipment`/`cancelB2BShipment`, wired at `POST /api/b2b/admin/orders/:id/create-shipment|cancel-shipment`) builds a small throwaway adapter object mapping `B2BOrder`'s field names onto the plain shape the retail-oriented service expects (`shippingAddress.contactName → name`, `items[].units → qty`, `totals.payable → total`, `paymentMethod: 'ONLINE'` always — **B2B never uses Shadowfax's own COD cash-collection**; the remainder is invoiced/collected by us directly, never by the courier). Same guard order and same persist-the-real-error-to-`courier.error`-on-failure pattern as retail's `shippingController.createShipment`.
- **Always admin-triggered** via the "Create Shipment" button (shown once an order is `dispatched` and has no AWB yet) — never automatic on order placement, same as retail.
- **Deliberately no B2B-specific weight cap** before calling Shadowfax (unlike retail's 7kg `SHADOWFAX_MAX_ORDER_WEIGHT_GRAMS` check) — a wholesale order can legitimately weigh far more than a retail parcel, and there's no confirmed real Shadowfax per-shipment limit to encode here without guessing. A genuine rejection surfaces via the normal `courier.error` path instead.
- **One shared webhook, not two**: rather than registering a second Shadowfax Push Callback URL (unconfirmed whether this Shadowfax account can even have one), `shippingController.handlePushCallback` now tries a retail `Order` lookup first, then falls back to `B2BOrder` (by `order_id` then by AWB, same as retail). Courier tracking state/history updates identically either way; the **status effect** deliberately differs — B2B's status enum/workflow doesn't map onto retail's placed/confirmed/processing/shipped states and is admin-driven rather than auto-progressing, so only a real `delivered` event, and only from the one legitimate predecessor state `dispatched`, auto-advances `B2BOrder.status`. Every other event still updates `courier.status`/`history`, just doesn't move the order's own status.
- **UI**: `B2BOrdersTab`'s order detail drawer shows "Create Shipment" once dispatched, then AWB/status/tracking-link + "Cancel shipment" once booked, with any booking error shown inline. `B2BOrderDetailPage` (business-facing) shows the same AWB/status/tracking read-only, in its own "Shipment tracking" card — separate from the older free-text `dispatch` card, which stays as-is regardless of whether a real shipment was ever booked.

### Invoicing, credit notes, and the ledger

- **Auto-issuance on dispatch**: `updateOrderStatus`'s `dispatched` branch calls `issueInvoiceForOrder` if the order has no invoice yet. The old "prepaid account with outstanding dues requires `force: true` to dispatch" confirm-dialog mechanism is gone (backend branch and the frontend `window.confirm`-retry code both removed) — it's structurally impossible to hit now that every order's advance is pre-verified via Razorpay at creation, not checked after the fact.
...
| POST | `/orders/quote` | `requireApprovedBusiness`, extra `b2bQuoteLimiter`. Server-priced preview, never charges. Response includes `advancePercent`/`advanceAmount`/`remainingAmount`. |
| POST | `/orders/payment/create-order` | `requireApprovedBusiness`, extra `b2bQuoteLimiter`. Creates a Razorpay order for the advance only + a `VerifiedPayment` row (same model retail uses). `400` if the account's advance would round to 0 — nothing to pay, place directly instead. |
| POST | `/orders` | `requireApprovedBusiness`. Places the order (`status: placed`). Body gains an optional `razorpayOrderId` — required (enforced in `utils/b2bOrderCreation.js`, not the validator) whenever the account's advance is nonzero. |
...
| POST | `/accounts/:id/approve\|reject\|suspend\|reactivate` | Approve requires `tier`+`advancePercent` (0 is valid — a pure-credit account); reject/suspend take a note/reason. |
...
| POST | `/orders/:id/status` | Body `{status, note, reason, dispatch}` — the state-machine + auto-invoice + credit-note-on-cancel logic. |
| POST | `/orders/:id/override-credit-hold` | Clears `creditHold`, logs who/when/why. |
| POST | `/orders/:id/invoice` | Manual invoice issuance. |
| POST | `/orders/:id/create-shipment` | Books a real Shadowfax AWB (see Shadowfax shipment above). `502` with the real Shadowfax error persisted to `courier.error` on failure. |
| POST | `/orders/:id/cancel-shipment` | Cancels the Shadowfax shipment (optional `remarks`). |
...
- **Pages** (`client/src/pages/business/`): `BusinessLandingPage` (public, indexable), `BusinessApplyPage`, `B2BDashboardPage` (status gate + credit summary + recent orders), `B2BQuickOrderPage` (grouped catalog, 400ms-debounced live quote, mobile sticky summary bar, `nc_b2b_draft_<userId>` localStorage draft, real Razorpay Checkout for the advance when one is due — see Advance/remainder payment above), `B2BOrdersPage`/`B2BOrderDetailPage` (status timeline, cancel, reorder-via-draft, invoice PDF download, real shipment tracking once booked), `B2BInvoicesPage`, `B2BStatementPage` (date-range filter, CSV export), `B2BProfilePage`.
...
- **Admin tabs** (`client/src/components/admin/`): `B2BAccountsTab` (approve form takes an `advancePercent` number input, not a payment-terms dropdown), `B2BCatalogTab` (tiers + wholesale catalog items), `B2BOrdersTab` (status actions, issue-invoice, create/cancel Shadowfax shipment once dispatched), `B2BLedgerTab` (FY-turnover-vs-threshold card, aging buckets, top-outstanding shortcut, account search/statement, payment/adjustment/opening-balance modals, invoice list + credit-note issuance), `B2BDisabledNotice`. Wired into `adminConstants.js` (`TABS`, `group: 'b2b'`), `AdminNav.jsx` (separate "Wholesale" section), `AdminPage.jsx` (tab render switch).
...
**No new environment variables for the advance-payment/Shadowfax-B2B work** — it reuses retail's existing `RAZORPAY_KEY_ID`/`RAZORPAY_KEY_SECRET`/`VITE_RAZORPAY_KEY_ID` and `SHADOWFAX_AUTH_TOKEN`/`SHADOWFAX_ENV`/`SHADOWFAX_WEBHOOK_TOKEN` (§28) as-is — same Razorpay account, same Shadowfax account, same webhook token, just a second consumer of each.
...
32. `B2BOrder.advancePercent` is `required: true` with no schema `default` used at write time by any current code path (only a genuinely new document sets it). Any test or script that does `B2BOrder.create({...})` directly (bypassing `utils/b2bOrderCreation.js`) must set `advancePercent` itself or the write throws `ValidationError: Path 'advancePercent' is required.` — this broke `tests/b2bCredit.test.js`'s own order fixture the first time `advancePercent` was added to the schema, since that fixture predated the field.
33. Two independent `Date.now()` reads a few milliseconds apart (e.g. `order.createdAt`, stamped by Mongoose inside a transaction, vs. a `remainingDueDate` computed just before it opens) are never bit-for-bit equal even though they're conceptually "the same moment" — assert a generous tolerance (milliseconds, not exact equality) when a test compares two independently-taken timestamps that are supposed to represent one instant.
34. The Shadowfax Push Callback webhook (`shippingController.handlePushCallback`) now resolves either a retail `Order` or a `B2BOrder` from the same payload/URL — if you add a third order type that can get a real AWB, extend the same fallback chain there rather than registering another webhook URL (unconfirmed whether this Shadowfax account even supports more than one).
