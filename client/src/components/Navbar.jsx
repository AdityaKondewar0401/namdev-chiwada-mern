...
export default function Navbar() {
  ...
  return (
    <>
      {/* ── Announcement Bar ── */}
      <div
        className={`w-full bg-[#1a3a2a] text-white text-xs font-medium py-2 px-6 flex items-center justify-between overflow-hidden transition-all duration-200 ${menuOpen ? "invisible opacity-0" : "visible opacity-100"}`}
      >
          ...
          >
            {[...Array(4)].map((_, i) => (
              <span key={i} className="inline-flex items-center gap-2 px-8">
                Welcome to the only official site of Namdev Chiwada. Please
                beware of other fraudulent sites with our name.
      ...
      {/* ── Main Navbar ── */}
      <nav
        className={`sticky top-0 left-0 right-0 z-40 bg-white border-b border-gray-100 shadow-sm transition-all duration-200 ${menuOpen ? "invisible opacity-0" : "visible opacity-100"}`}
      >
        <div
        ...
        >
          ...
          <div className="hidden md:flex items-center gap-1 mx-auto">
            {NAV_LINKS.map((l) => (
              ...
              >
                {l.label}
                <span
                  className={`absolute bottom-0 left-0 right-0 h-0.5 bg-saffron transition-all duration-300 ${
                    isActive(l.to)
                      ? "opacity-100"
          ...
          <div className="hidden md:flex items-center gap-4 flex-shrink-0">
            ...
            <Link
              to="/cart"
              className="relative p-2 text-brown-dark hover:text-saffron transition-colors"
            >
              <svg
                xmlns="http://www.w3.org/2000/svg"
                className="w-6 h-6"
            ...
            {user ? (
              <div className="relative group">
                <button className="flex items-center gap-2 px-4 py-2 rounded-full transition-all duration-200 hover:bg-saffron/8 border border-transparent hover:border-saffron/20">
                  ...
                  <svg
                    className="w-3.5 h-3.5 text-brown-mid/60 transition-transform duration-200 group-hover:rotate-180"
                    fill="none"
                    viewBox="0 0 24 24"
                    stroke="currentColor"
                    strokeWidth={2.5}
                ...
                <div
                  className="absolute right-0 top-full mt-2 w-56 rounded-2xl overflow-hidden opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 z-50"
                  style={{
                    boxShadow: "0 16px 48px rgba(45,26,0,0.15)",
                    border: "1px solid rgba(224,112,0,0.1)",
                    background: "#fff",
                  }}
                >
                  <div
                    className="px-4 py-3 border-b border-saffron/10"
                    style={{
                      background: "linear-gradient(135deg,#fff0d6,#fffdf7)",
                    }}
                  >
                    <div className="font-bold text-brown-dark text-sm truncate">
                      {user.name}
                    </div>
                    <div className="text-xs text-brown-mid/60 truncate">
                      {user.email}
                    </div>
                  </div>
                  <div className="py-2">
              ...
              <Link
                to="/login"
                className="text-sm font-semibold text-brown-mid hover:text-saffron transition-colors"
              >
                Login
              </Link>
            )}
          ...
          <div className="flex md:hidden items-center gap-2 ml-auto">
            ...
            >
              <span
                className={`block w-6 h-0.5 bg-brown-dark transition-all duration-300 ${menuOpen ? "translate-y-2 rotate-45" : ""}`}
              />
              <span
                className={`block w-6 h-0.5 bg-brown-dark transition-all duration-300 ${menuOpen ? "opacity-0" : ""}`}
              />
              <span
                className={`block w-6 h-0.5 bg-brown-dark transition-all duration-300 ${menuOpen ? "-translate-y-2 -rotate-45" : ""}`}
              />
          ...
          <>
            ...
            <motion.div
              ...
              style={{
                ...
                background:
                  "linear-gradient(160deg, #2d1a00 0%, #3d1c00 55%, #4a2200 100%)",
                  ...
                  <div className="flex flex-col gap-0.5">
                    ...
                    <div
                      ...
                      style={{
                        ...
                        border: "1px solid rgba(224,112,0,0.25)",
                      }}
                    >
                      <div
