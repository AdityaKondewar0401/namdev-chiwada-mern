...
export default function ProductFormTab({ editProduct, onSave, onCancel }) {
  ...
  const handleMainImageUpload = async (file) => {
    ...
    try {
      ...
      if (res.data.success) {
        ...
        if (oldPublicId) {
          api.delete(`/api/upload/${encodeURIComponent(oldPublicId)}`).catch(() => {});
        }
      } else {
  ...
  const removeUploadedImage = (index) => {
    ...
    // Best-effort Cloudinary cleanup — don't block the form on it.
    const publicId = publicIdFromUrl(removedUrl);
    if (publicId) {
      api.delete(`/api/upload/${encodeURIComponent(publicId)}`).catch(() => {});
    }
  };
