        ...
        {filtered.map((p) => (
          ...
          >
            ...
            <div className="flex-1 min-w-[140px]">
              ...
              <div className="flex items-center gap-1 text-xs text-brown-mid/60 mt-0.5">₹{p.price} · {p.category} · <Star size={11} className="fill-amber-400 text-amber-400" /> {p.rating} ({p.reviews})</div>
            ...
            <div className="flex gap-2 flex-shrink-0 w-full sm:w-auto">
              <button
                onClick={() => onEdit(p)}
                className="flex-1 sm:flex-none flex items-center justify-center gap-1.5 px-3 py-2 rounded-xl text-xs font-bold transition-all hover:-translate-y-0.5"
