...
export default function ProductCard({ product, index = 0 }) {
  ...
  return (
    <MotionLink
      ...
      <div className="relative overflow-hidden" style={{
        ...
        {product.badge && product.inStock && (
          <div className="absolute top-2 left-2 sm:top-3 sm:left-3 z-10">
            <span className="px-2.5 py-1 sm:px-3 sm:py-1 rounded-full text-[10px] sm:text-[11px] font-bold text-white shadow-md tracking-wide"
      ...
      {/* ── Body ─────────────────────────────────────── */}
      <div className="p-4 sm:p-4 flex flex-col flex-1 gap-2.5 sm:gap-2.5">

        {/* Name + rating + a touch of real product info, single tight block */}
        <div>
          {(product.sub || product.tag) && (
            <span className="text-[10px] sm:text-[10px] font-bold tracking-widest uppercase text-saffron">
              {product.sub || product.tag}
            </span>
          )}
          <h3 className="font-serif font-black text-brown-dark text-xl sm:text-lg leading-snug line-clamp-2 mt-0.5">
            {product.name}
          </h3>
          <div className="flex items-center gap-1.5 sm:gap-1 mt-1 sm:mt-1">
            <span className="text-amber-400 text-sm sm:text-xs tracking-tight">
              {'★'.repeat(Math.round(product.rating || 0))}
              <span className="text-brown-dark/15">{'★'.repeat(5 - Math.round(product.rating || 0))}</span>
            </span>
            <span className="text-xs sm:text-[11px] text-brown-mid/45">({product.reviews || 0})</span>
          </div>
          {(product.intro || product.desc) && (
            <p className="block text-xs text-brown-mid/60 sm:text-brown-mid/55 mt-2 sm:mt-1.5 leading-relaxed sm:leading-snug line-clamp-2 sm:line-clamp-1">
              {product.intro || product.desc}
            </p>
          )}
        </div>
        ...
        <div className="flex flex-col gap-2.5 sm:flex-row sm:items-center sm:justify-between sm:gap-3 mt-auto pt-1" onClick={(e) => e.stopPropagation()}>
          <div className="flex-shrink-0 flex items-baseline gap-1.5">
            ...
            {product.originalPrice && (
              <span className="text-xs sm:text-xs text-brown-mid/35 line-through">
                ₹{product.originalPrice}
              </span>
            )}
          </div>
