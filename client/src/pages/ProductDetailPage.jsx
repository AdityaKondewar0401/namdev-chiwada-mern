...
export default function ProductDetailPage() {
  ...
  const selectedSize = currentSize.weight;
  const qty = getItemQuantity(product._id, selectedSize);
  ...
  const handleAddToCart = () => {
    addToCart(product, selectedSize, currentSize.price, 1);
  };
  ...
  const handleDecrease = () => {
    if (qty === 1) {
      ...
      updateQuantity(product._id, selectedSize, qty - 1);
    }
  };
    ...
    <div className="min-h-screen bg-cream pb-16">
      <div className="max-w-6xl mx-auto px-6 pt-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-12 lg:gap-16 items-start">
          ...
          {/* Info */}
          <div>
            <div className="text-xs font-bold tracking-widest uppercase text-saffron mb-2">
              {product.sub}
            </div>

            <h1
              className="font-serif font-black text-brown-dark leading-tight mb-3"
              style={{
                fontSize:
            ...
            <div
              className="text-saffron font-black mb-4"
              style={{ fontSize: '2rem' }}
            >
              ₹{currentSize.price}

              <small className="text-base text-brown-mid/50 font-normal ml-1">
                /{currentSize.weight}
              </small>
            </div>

            <p className="text-brown-dark/70 leading-relaxed mb-6">
              {product.desc}
            </p>
            ...
            {product.sizes?.length > 0 && (
              <div className="mb-6">
                <div className="text-xs font-bold uppercase tracking-widest text-brown-dark mb-3">
                  Select Size
                </div>
            ...
            <div className="mb-8 flex gap-3 flex-wrap">
              {qty === 0 ? (
                ...
                <div className="flex-1 min-w-44 flex items-center justify-center gap-4 bg-orange-500 rounded-full py-3 px-4">
                  <button
                    onClick={handleDecrease}
                    className="text-white font-bold text-2xl w-10 h-10 flex items-center justify-center"
                  >
                    −
                  </button>

                  <span className="text-white font-bold text-xl min-w-[2rem] text-center">
                    {qty}
                  </span>
                ...
                <div className="flex flex-wrap gap-2">
                  {(product.ingredients || []).map(
                    (ing) => (
                      <span
                        key={ing}
                        className="px-3 py-1.5 rounded-full text-xs text-brown-dark bg-cream-mid border border-saffron/15"
                      >
                        {ing}
                      </span>
                    )
                  ...
                  <tbody>
                    {(product.nutrition || []).map(
                      ([label, val]) => (
                        <tr
                          key={label}
                          className="border-b border-saffron/8"
                        >
                          <td className="py-2.5 text-brown-mid/70 font-medium">
                            {label}
                          </td>

                          <td className="py-2.5 font-bold text-brown-dark text-right">
                            {val}
                          </td>
                        </tr>
                      )
        ...
        {related.length > 0 && (
          <div className="mt-20">
            <div className="mb-8">
              ...
              <h3 className="font-serif font-bold text-brown-dark text-2xl">
                Related Products
              </h3>
            </div>
