  ...
  */
  ...
  try {
    ...
    */
    ...
    // Log the real reason server-side (expired vs malformed vs wrong
    // secret vs a genuine bug) — this was previously swallowed with zero
    // visibility, meaning e.g. a misconfigured JWT_SECRET in production
    // would silently 401 every single request with no trace in the logs.
    // The client-facing message stays generic either way.
    console.error(`JWT verification failed on ${req.method} ${req.originalUrl}:`, err.message);
    return res.status(401).json({
      success: false,
