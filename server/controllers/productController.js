...
// literally — an unescaped `q` here is a public, unauthenticated ReDoS
// surface (ReDoS-crafted patterns against `desc`/`intro` text).
...
exports.updateProduct = async (req, res, next) => {
  try {
    ...
    // findByIdAndUpdate skips the pre-save slug hook, so a rename needs an
    // explicit slug regeneration here — otherwise the product's URL keeps
    // pointing at its old name indefinitely.
    if (update.name && update.name !== existing.name && !update.slug) {
...
exports.searchProducts = async (req, res, next) => {
  try {
    ...
    const safeQuery = escapeRegex(q.slice(0, 100));

    const products = await Product.find({
      $or: [
        { name:  { $regex: safeQuery, $options: 'i' } },
        { sub:   { $regex: safeQuery, $options: 'i' } },
        { desc:  { $regex: safeQuery, $options: 'i' } },
        { intro: { $regex: safeQuery, $options: 'i' } },
      ],
    }).limit(10);
