...
// over the same email — this is the most likely cause of inconsistent
// login behavior (e.g. "password sometimes missing" after Google login).
...
async function run(emailArg) {
  ...
  if (users.length > 1) {
    ...
    console.log('   behavior (password sometimes "missing" after Google login).');
  } else if (users.length === 1) {
    console.log('Only one account exists — good, nothing to fix.');
  } else {
